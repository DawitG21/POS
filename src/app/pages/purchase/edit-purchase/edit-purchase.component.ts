...
export class EditPurchaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.busy = true;
    this.purchase = this.storageService.getData(this.keyService.PURCHASE_KEY);
    this.storageService.deleteData(this.keyService.PURCHASE_KEY);
    ...
    this.validateCalls();
    this.populateData(this.purchase);
    this.busy = false;
  }
  ...
  populateData(arg: Purchase) {
    this.form.get("id")!.setValue(arg._id);
    ...
    this.form.get("purchaseDate")?.setValue(formattedDate);
  ...
  edit(purchase: Purchase) {
    return lastValueFrom(
      this.purchaseService.edit(purchase, purchase.transactionId)
    );
  }
  ...
  save() {
    ...
    const transactionId = this.purchase.transactionId;
    ...
    this.model.purchaseCode = this.form.get("purchaseDate")?.value!;
    this.model.discount = +this.form.get("discount")?.value!;
    this.model.actionType = this.form.get("actionType")?.value!;
    this.model.puchaseDate = this.form.get("purchaseDate")?.value!;
    this.model.transactionId = transactionId;
    this.model.companyId = this.purchase.company.id!;
    this.model._id = this.form.get("id")?.value!;
